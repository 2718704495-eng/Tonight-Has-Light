...
const EXCLUDED = new Set(['HASHES.sha256', '.DS_Store']);
...
for (const file of files) {
  ...
  lines.push(`${digest}  ${relative(PACKAGE_ROOT, file)}`);
}
await writeFile(resolve(PACKAGE_ROOT, 'HASHES.sha256'), `${lines.join('\n')}\n`);
