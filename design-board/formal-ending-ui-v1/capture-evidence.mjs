...
const baseUrl = process.env.ENDING_UI_BASE_URL || "http://127.0.0.1:4181/design-board/formal-ending-ui-v1/";
const chromePath = process.env.CHROME_PATH || "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome";

const cases = [
  ["a-ending-360x800", 360, 800, "a-ending"],
  ["a-ending-390x844", 390, 844, "a-ending"],
  ["a-summary-430x932", 430, 932, "a-summary"],
  ["a-ending-430x844-pressure", 430, 844, "a-ending"],
  ["b-ending-390x844", 390, 844, "b-ending"],
  ["b-summary-430x932", 430, 932, "b-summary"],
  ["b-large-360x800", 360, 800, "b-large"],
  ["b-large-430x844-pressure", 430, 844, "b-large"],
  ["a-ending-reduced-390x844", 390, 844, "a-ending", { reducedMotion: true }],
  ["share-390x844", 390, 844, "share"],
  ["failure-390x844", 390, 844, "failure"],
];

function startChrome() {
  const profile = mkdtempSync(join(tmpdir(), "ending-ui-cdp-"));
  const child = spawn(
    chromePath,
    [
      "--headless=new",
  ...
  const endpoint = new Promise((resolve, reject) => {
    let stderr = "";
    const timer = setTimeout(() => reject(new Error(`Chrome DevTools endpoint timeout\n${stderr}`)), 10000);
...
async function newTarget(browserEndpoint, url) {
...
async function captureCase(browserEndpoint, [name, width, height, view, options = {}]) {
  const target = await newTarget(browserEndpoint, "about:blank");
  ...
  if (metrics.reducedMotion !== Boolean(options.reducedMotion)) {
  ...
  return {
    ...
    expectedReducedMotion: Boolean(options.reducedMotion),
    ...metrics,
  };
}
...
try {
  ...
  const results = [];
  for (const item of cases) results.push(await captureCase(browserEndpoint, item));
  writeFileSync(join(evidenceDir, "viewport-metrics.json"), `${JSON.stringify(results, null, 2)}\n`);
  results.forEach((result) => {
    console.log(
      `PASS ${result.name}: inner=${result.innerWidth}x${result.innerHeight}, overflow=${result.horizontalOverflow}, reduced=${result.reducedMotion}`,
    );
  });
} finally {
